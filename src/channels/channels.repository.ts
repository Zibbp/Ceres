...
export class ChannelsRepository extends Repository<Channel> {
    ...
    async createChannel(internalCreateChannel: InternalCreateChannelDto): Promise<Channel> {
        const { id, login, displayName, description, profileImagePath, offlineImagePath, channelCreatedAt } = internalCreateChannel;
        let channel: Channel;
        try {
            channel = this.create({
                id,
